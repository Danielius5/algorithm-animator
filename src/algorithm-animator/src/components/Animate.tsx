...
export function Animate({states, sequence, id}:AnimateParams) {
    ...
    return (
        <>
            <label htmlFor={"user-input-text-for-dfa" + id}>Sequence to match:</label> <br/>
            <input disabled={sequence !== undefined} onChange={(e) => {setText(e.target.value);  setCurrentStep(0);setCurrentLetter(0.5);}} value={text} id={"user-input-text-for-dfa" + id} />
            <br/>
